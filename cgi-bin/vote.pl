...
if ( $new && !( $gene_name || $dbs_id ) ) {
    print h1(
        font(
            { color => 'red' },
            'Either a gene name or database ID is required'
        )
    );
    $new    = '';
    $create = 1;
...
my @tfvinfo = (
    $usr_initial,     $gene_name, $dbs_id,   $tag_loc,
    $tag,             $sty_pp,    $ab_avail, $mut_avail,
    $construct_avail, 1
);
for (@tfvinfo) {
    s/\t/ /g;
...
while ( my $line = <IN> ) {
    ...
    push @columns,
qq(<input type="radio" name="vote" value="$vote_idx" onclick="document.f1.submit()">);
